    ...
    def __init__(self, providers_df, config_path=None):
        ...
        if config_path is None:
            config_path = os.path.abspath(
                os.path.join(os.path.dirname(__file__), 'config', 'UC07_STEP7_ADVANCED_MATCHING_CONFIG.json')
            )
        
        try:
            with open(config_path, 'r') as f:
                self.config = json.load(f)
            print(f"[AdvancedProviderMatchingEngine] Config loaded from: {config_path}")
        except Exception as e:
            print(f"[AdvancedProviderMatchingEngine] WARNING: Could not load config at '{config_path}': {e}. Using fallback defaults.")
